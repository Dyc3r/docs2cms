...
class TestSyncDocument:
    def test_creates_new_document_and_updates_frontmatter(self, tmp_path, cfg):
        ...
        with respx.mock:
            ...
            _sync_document(doc_file, cfg)
        post = frontmatter.load(doc_file)
        assert post.metadata["wordpress_id"] == 101

    def test_updates_existing_document_via_item_route(self, tmp_path, cfg):
    ...
    def test_skips_sync_when_hash_matches(self, tmp_path, cfg):
        doc_file = _synced_doc(tmp_path, wp_id=5)
        # No routes registered — any HTTP request would raise ConnectError
        with respx.mock:
            _sync_document(doc_file, cfg)

    def test_updates_document_hash_in_frontmatter(self, tmp_path, cfg):
        doc_file = _new_doc(tmp_path)
    ...
    def test_deletes_deprecated_document(self, tmp_path, cfg):
        doc_file = _write_doc(
            tmp_path,
            f"---\ndocument_key: {DOC_KEY}\ntitle: Old\nslug: old\ncontent_type: docs\n"
            "wordpress_id: 99\ndeprecated: true\n---\nOld content\n",
    ...
    def test_deprecated_doc_never_synced_is_just_removed_locally(self, tmp_path, cfg):
        doc_file = _write_doc(
            tmp_path,
            f"---\ndocument_key: {DOC_KEY}\ntitle: Ghost\nslug: ghost\ncontent_type: docs\n"
            "wordpress_id: \ndeprecated: true\n---\nContent\n",
    ...
    def test_syncs_tags_for_new_document(self, tmp_path, cfg):
        doc_file = _write_doc(
            tmp_path,
            f"---\ndocument_key: {DOC_KEY}\ntitle: Tagged\nslug: tagged\ncontent_type: docs\n"
            "parent_key: \ntags: [python, cms]\nwordpress_id: \n"
    ...
    def test_resolves_parent_before_posting(self, tmp_path, cfg):
        doc_file = _write_doc(
            tmp_path,
            f"---\ndocument_key: {DOC_KEY}\ntitle: Child\nslug: child\ncontent_type: docs\n"
    ...
    def test_raises_when_parent_not_found(self, tmp_path, cfg):
        doc_file = _write_doc(
            tmp_path,
            f"---\ndocument_key: {DOC_KEY}\ntitle: Child\nslug: child\ncontent_type: docs\n"
            f"parent_key: {PARENT_KEY}\ntags: []\nwordpress_id: \n"
...
class TestSyncDirectory:
    def test_syncs_each_file_in_directory(self, tmp_path, cfg):
    ...
    def test_recurses_into_subdirectories(self, tmp_path, cfg):
    ...
    def test_empty_directory_makes_no_sync_calls(self, tmp_path, cfg):
        with patch("d2cms.wordpress._sync_document") as mock_sync:
            _sync_directory(tmp_path, cfg)
        mock_sync.assert_not_called()

    def test_passes_cfg_to_sync_document(self, tmp_path, cfg):
        doc = tmp_path / "doc.md"
        doc.write_text("content")
        with patch("d2cms.wordpress._sync_document") as mock_sync:
            _sync_directory(tmp_path, cfg)
        mock_sync.assert_called_once_with(doc, cfg)

    def test_deeply_nested_structure(self, tmp_path, cfg):
        deep = tmp_path / "a" / "b" / "c"
    ...
    def test_parent_doc_synced_before_subdirectory(self, tmp_path, cfg):
        ...
        with patch(
            ...
            side_effect=lambda path, _: call_order.append(path),
        ):
            _sync_directory(tmp_path, cfg)
...
class TestSync:
    def test_sync_calls_sync_directory_with_docs_dir(self, cfg):
        with patch("d2cms.wordpress._sync_directory") as mock_dir:
            sync(cfg)
        mock_dir.assert_called_once_with(cfg.docs_dir, cfg)
